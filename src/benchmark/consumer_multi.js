...
async function consumer(consumerId) {
  const q = new Queen({
  ...
  const consumerMetrics = {
    ...
    startTime: Date.now(),
    endTime: null,
    queuesConsumed: new Set(),
    partitionsConsumed: new Set()
  };

  const target = `namespace:${NAMESPACE}`;
  
  metrics.consumers.push(consumerMetrics);
  console.log(`[Consumer ${consumerId}] Started, consuming from namespace '${NAMESPACE}'`);
  
  try {
    for await (const messages of q.takeBatch(target, {
      wait: true,
      timeout: 15000,
      batch: BATCH_SIZE,
      idleTimeout: 5000
      ...
      try {
        // Track which queues and partitions this consumer is consuming from
        for (const message of messages) {
          if (message.queue) consumerMetrics.queuesConsumed.add(message.queue);
          if (message.partition !== undefined) consumerMetrics.partitionsConsumed.add(`${message.queue}:${message.partition}`);
        }
        
        // Validate message ordering per queue/partition
        const queuePartitionLastIds = new Map();
        ...
        for (const message of messages) {
          ...
          // Use queue and partition to ensure proper ordering within each queue/partition
          const queuePartitionKey = `${message.queue || 'default'}:${message.partition || 'default'}`;
          const messageId = message.data.id;
          
          if (queuePartitionLastIds.has(queuePartitionKey)) {
            const lastId = queuePartitionLastIds.get(queuePartitionKey);
            if (messageId !== lastId + 1) {
              console.log(`Queue: ${message.queue}, Partition: ${message.partition}, Message:`, message);
              throw new Error(`Message ordering violation in '${queuePartitionKey}': expected id ${lastId + 1}, got ${messageId}`);
            }
          }
          
          queuePartitionLastIds.set(queuePartitionKey, messageId);
        }
...
// Launch all consumers (they will all consume from the namespace)
console.log(`Launching ${NUMBER_OF_CONSUMERS} consumers for namespace '${NAMESPACE}'...\n`);
const consumerPromises = [];
for (let i = 0; i < NUMBER_OF_CONSUMERS; i++) {
  consumerPromises.push(consumer(i));
}
...
metrics.consumers.forEach(c => {
  ...
  const queuesStr = c.queuesConsumed.size > 0 ? `queues:[${Array.from(c.queuesConsumed).sort().join(',')}]` : '';
  const partitionsStr = c.partitionsConsumed.size > 0 ? `partitions:${c.partitionsConsumed.size}` : '';
  const consumedFrom = [queuesStr, partitionsStr].filter(Boolean).join(' ');
  
  console.log(`  [C${c.consumerId}] ${c.messagesProcessed.toLocaleString()} msgs | Active: ${actualProcessingTime.toFixed(2)}s (${rate} msg/s) | Idle: ${idleTime}s | ${c.batchesProcessed} batches | avg: ${avgBatch}s | ${consumedFrom}`);
});
