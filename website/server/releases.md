...
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **0.13.0**     | Major release. (1) New libqueen with an adaptive batch/concurrency/scheduling engine - S1 pushes ~2x, S3 pushes ~3x at the same p99, PG pinned instead of idle. (2) New `push_messages_v3` stored procedure: temp-table-free, single JSONB parse, `ON CONFLICT DO NOTHING` dedup. Same external response shape as v2. (3) New dashboard / frontend (Vue 3, queue and analytics views, DLQ tools, maintenance switch). (4) Server-stamped `producerSub` from the validated JWT `sub` on every message - closes impersonation vector (issue #23). | All ≥0.12.x work unchanged - 0.13.0 pop responses just add a new `producerSub` field that older clients silently ignore. Upgrade to 0.13.0 clients only if you want typed access to `producerSub` (Go `Message.ProducerSub` field, Python `TypedDict` hint) |
...
- Server 0.13.0: **New libqueen with adaptive engine.** The per-worker push/ack drain is factored into three independently-tuned concerns - batching, concurrency, and scheduling - glued by an event-driven orchestrator (design notes in `cdocs/LIBQUEEN_IMPROVEMENTS.md`). Fixes two long-standing bottlenecks surfaced by the perf harness: per-commit overhead amortization on small-batch workloads (S1) and the single-slot-per-drain cap on high-fanout workloads (S3). Headline numbers against the previous baseline: S1 ~6.2k → ~13k pg_ins/s, S3 ~4.7k → ~20k pg_ins/s, with PG pinned instead of idle.
- Server 0.13.0: **New push stored procedure.** `queen.push_messages_v2` rewritten around a temp-table + batched insert pipeline that feeds cleanly into the new adaptive engine; preserves the existing HTTP contract (queued/duplicate/failed semantics unchanged).
- Server 0.13.0: **New dashboard / frontend.** Vue 3 rewrite covering queues, analytics, DLQ management, and maintenance mode; served by the same C++ acceptor at `/`.
...
- Clients 0.13.0: All clients (JS, Python, Go, Laravel, C++) expose `producerSub` on popped messages; Go adds a typed `Message.ProducerSub` field and the hand-rolled JSON parser was updated to read it.
