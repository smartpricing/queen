...
-- ============================================================================
-- queen.get_status_v1: Dashboard overview with optional filters
-- Returns full analytics data including throughput time series for MessageFlowChart
-- ============================================================================
CREATE OR REPLACE FUNCTION queen.get_status_v1(p_filters JSONB DEFAULT '{}'::jsonb)
RETURNS JSONB
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    -- Calculate dynamic bucket size (matching C++ logic)
    ...
    END;
    v_bucket_seconds := v_bucket_minutes * 60;
    
    -- Query 1: Throughput time series with dynamic bucketing
    WITH bucketed_ingested AS (
        SELECT 
            TO_TIMESTAMP(FLOOR(EXTRACT(EPOCH FROM m.created_at) / v_bucket_seconds) * v_bucket_seconds) AS bucket,
    ...
    bucketed_processed AS (
        SELECT 
            TO_TIMESTAMP(FLOOR(EXTRACT(EPOCH FROM mc.acked_at) / v_bucket_seconds) * v_bucket_seconds) AS bucket,
        ...
        JOIN queen.queues q ON q.id = p.queue_id
        WHERE mc.acked_at >= v_from_ts
            AND mc.acked_at <= v_to_ts
            AND (v_queue IS NULL OR q.name = v_queue)
            AND (v_namespace IS NULL OR q.namespace = v_namespace)
            AND (v_task IS NULL OR q.task = v_task)
        GROUP BY bucket
    ...
    SELECT 
        COALESCE(jsonb_agg(
            jsonb_build_object(
                'timestamp', to_char(ab.bucket, 'YYYY-MM-DD"T"HH24:MI:SS"Z"'),
                'ingested', COALESCE(bi.messages_ingested, 0),
                'processed', COALESCE(bp.messages_processed, 0),
                'ingestedPerSecond', ROUND(COALESCE(bi.messages_ingested, 0)::numeric / v_bucket_seconds, 2),
                'processedPerSecond', ROUND(COALESCE(bp.messages_processed, 0)::numeric / v_bucket_seconds, 2)
    ...
    WITH active_queues AS (
        ...
        LEFT JOIN queen.messages m ON m.partition_id = p.id
            AND m.created_at >= v_from_ts AND m.created_at <= v_to_ts
    ...
    ), '[]'::jsonb) INTO v_queues FROM active_queues;
    ...
    SELECT jsonb_build_object(
        'total', COUNT(DISTINCT tfm.id),
        'pending', GREATEST(0, COUNT(DISTINCT CASE 
        ...
        END) - COUNT(DISTINCT CASE 
            ...
            ELSE NULL 
        END)),
        'processing', COUNT(DISTINCT CASE 
            WHEN pc.lease_expires_at IS NOT NULL 
                AND pc.lease_expires_at > NOW()
                AND (
                    pc.last_consumed_created_at IS NULL 
        ...
        END),
        'failed', 0,
        'deadLetter', COUNT(DISTINCT CASE 
    ...
    -- Query 5: DLQ stats with top errors
    WITH dlq_stats AS (
        SELECT 
            dlq.partition_id,
            dlq.error_message,
        ...
        LEFT JOIN queen.queues q ON q.id = p.queue_id
        WHERE dlq.failed_at >= v_from_ts AND dlq.failed_at <= v_to_ts
            AND (v_queue IS NULL OR q.name = v_queue)
            AND (v_namespace IS NULL OR q.namespace = v_namespace)
            AND (v_task IS NULL OR q.task = v_task)
        GROUP BY dlq.partition_id, dlq.error_message
    )
    SELECT jsonb_build_object(
        'totalMessages', COALESCE(SUM(error_count), 0),
        'affectedPartitions', COUNT(DISTINCT partition_id),
        'topErrors', COALESCE((
            SELECT jsonb_agg(
                jsonb_build_object('error', error_message, 'count', error_count)
            )
            FROM (
                SELECT error_message, SUM(error_count) as error_count
                FROM dlq_stats
    ...
    ) INTO v_dlq FROM dlq_stats;
    
    -- Handle case when no DLQ data
...
-- ============================================================================
-- queen.get_status_queues_v1: Queues list with stats
-- ============================================================================
CREATE OR REPLACE FUNCTION queen.get_status_queues_v1(
...
-- ============================================================================
-- queen.get_queue_detail_v1: Queue detail with partition stats
-- ============================================================================
CREATE OR REPLACE FUNCTION queen.get_queue_detail_v1(p_queue_name TEXT)
RETURNS JSONB
LANGUAGE plpgsql
AS $$
DECLARE
...
GRANT EXECUTE ON FUNCTION queen.get_status_v1(JSONB) TO PUBLIC;
GRANT EXECUTE ON FUNCTION queen.get_status_queues_v1(JSONB, INTEGER, INTEGER) TO PUBLIC;
GRANT EXECUTE ON FUNCTION queen.get_queue_detail_v1(TEXT) TO PUBLIC;
