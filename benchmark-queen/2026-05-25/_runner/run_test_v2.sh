...
log "=== START test=$TEST_NAME maxPart=$MAX_PARTITION batch=$MSGS_PER_PUSH dur=${DURATION}s prod=${PROD_WORKERS}x${PROD_CONN} cons=${CONS_WORKERS}x${CONS_CONN} consBatch=$CONS_BATCH ==="
...
QUEUE_NAME="$QUEUE_NAME" NUM_WORKERS="$PROD_WORKERS" CONNECTIONS_PER_WORKER="$PROD_CONN" \
...
QUEUE_NAME="$QUEUE_NAME" NUM_WORKERS="$CONS_WORKERS" CONNECTIONS_PER_WORKER="$CONS_CONN" \
