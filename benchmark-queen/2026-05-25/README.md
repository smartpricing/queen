...
> **Realistic pipeline benchmark** (4-stage producer → worker → analytics + log fan-out, real Node clients via pm2):
>
> - Per-entity ordering config (1 000 partitions): `[pipeline-queen.md](./pipeline-queen.md)` — **3 688 msg/s** sustained end-to-end, p99 = 1.02 s, 99.96 % delivery completeness, 0 duplicates.
> - Throughput-tuned config (10 partitions, batch=1 000): `[pipeline-queen-throughput.md](./pipeline-queen-throughput.md)` — **6 673 msg/s** (+81 %) sustained end-to-end, p99 = 1.10 s, 9× tighter max latency, 3.2× lower Postgres CPU. Same engine, same pipeline shape, partition knob slid to the other end.
