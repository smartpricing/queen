-- Migration: Add push_messages stored procedure for high-performance batch push
-- This eliminates multiple round trips by doing all push logic in a single DB call
...
CREATE OR REPLACE FUNCTION queen.push_messages_v2(
    p_items jsonb,           -- Array of {queue, partition, payload, transactionId, traceId, namespace, task}
...
DECLARE
    ...
    v_partition_map jsonb := '{}'::jsonb;  -- Maps "queue:partition" -> partition_id
    v_queue_map jsonb := '{}'::jsonb;      -- Maps queue_name -> {id, max_queue_size}
    v_capacity_map jsonb := '{}'::jsonb;   -- Maps queue_name -> current_depth
...
BEGIN
    ...
    -- ============================================================
    -- PHASE 2: Ensure all queues exist and get their IDs/config
    -- ============================================================
    
    FOR v_queue_name IN SELECT jsonb_object_keys(v_queue_map) LOOP
        -- Get first item for this queue to extract namespace/task
        ...
        -- Upsert queue and get ID + config
        INSERT INTO queen.queues (name, namespace, task)
        VALUES (v_queue_name, v_namespace, v_task)
        ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name  -- No-op update to get RETURNING to work
    ...
    -- ============================================================
    -- PHASE 3: Ensure all partitions exist and get their IDs
    -- ============================================================
    ...
    -- ============================================================
    -- PHASE 4: Check capacity for all queues (if enabled)
    -- ============================================================
    ...
    IF p_check_capacity THEN
        FOR v_queue_name IN SELECT jsonb_object_keys(v_queue_map) LOOP
            ...
            IF v_max_queue_size IS NOT NULL AND v_max_queue_size > 0 THEN
                ...
                -- Count pending messages for this queue (messages not yet consumed)
                ...
                -- Count batch size for this queue
    ...
    -- ============================================================
    -- PHASE 5: Build insert arrays (skip capacity-exceeded queues)
    -- ============================================================
    ...
    FOR v_idx IN 0..jsonb_array_length(p_items) - 1 LOOP
        ...
        -- Add to insert arrays
        -- Use messageId from payload (UUIDv7 generated by C++) or fallback to random
    ...
    -- ============================================================
    -- PHASE 6: Check duplicates (if enabled and we have items to insert)
    -- ============================================================
    
    IF p_check_duplicates AND array_length(v_transaction_ids, 1) > 0 THEN
        -- Find existing messages with same transaction_id in same partition
        ...
        IF v_existing_dups IS NOT NULL THEN
            -- Filter out duplicates from insert arrays
            DECLARE
    ...
    -- ============================================================
    -- PHASE 7: Batch INSERT remaining messages
    -- ============================================================
    ...
    -- INSERT...RETURNING doesn't guarantee order, so we must sort explicitly
...
CREATE INDEX IF NOT EXISTS idx_messages_txn_partition 
...
COMMENT ON FUNCTION queen.push_messages_v2 IS 
'High-performance batch push that handles queue/partition creation, capacity checks, 
duplicate detection, and message insertion in a single database call.

Parameters:
  p_items: JSON array of objects with keys:
    - queue (required): Queue name
    - partition (optional, default "Default"): Partition name  
    - payload (optional, default {}): Message payload as JSON
    - transactionId (optional, auto-generated UUID): Unique message ID for deduplication
    - traceId (optional): Trace ID for distributed tracing
    - namespace (optional, derived from queue): Queue namespace
    - task (optional, derived from queue): Queue task

  p_check_duplicates: If true, skip messages with existing transaction_id in same partition
  p_check_capacity: If true, reject messages if queue would exceed max_queue_size

Returns: JSON array of result objects with keys:
  - index: Original position in input array
  - transaction_id: The transaction ID used
  - status: "queued", "duplicate", "rejected", or "failed"
  - message_id: Database ID of inserted message (if queued)
  - partition_id: Partition UUID (if queued)
