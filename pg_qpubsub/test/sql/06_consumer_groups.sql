...
BEGIN
    ...
    -- Consumer group 1 pops messages
    SELECT COUNT(*) INTO count_g1 
    FROM queen.pop('test-pubsub-multi', 'consumer-group-1', 10, 60);
    
    -- Consumer group 2 also pops same messages (pub/sub fan-out)
    SELECT COUNT(*) INTO count_g2 
    FROM queen.pop('test-pubsub-multi', 'consumer-group-2', 10, 60);
...
BEGIN
    ...
    -- Group 1 pops and acks first message
    SELECT * INTO msg1 FROM queen.pop('test-pubsub-isolation', 'isolated-group-1', 1, 60);
    PERFORM queen.ack_group(msg1.transaction_id, msg1.partition_id, msg1.lease_id, 'isolated-group-1');
    
    -- Group 1 should have 1 remaining
    SELECT COUNT(*) INTO remaining_g1 
    FROM queen.pop('test-pubsub-isolation', 'isolated-group-1', 10, 60);
    
    -- Group 2 should still see all messages (hasn't consumed any)
    SELECT COUNT(*) INTO count_g2 
    FROM queen.pop('test-pubsub-isolation', 'isolated-group-2', 10, 60);
...
BEGIN
    ...
    FROM queen.pop('test-pubsub-partition', 'partition-a', 'partition-consumer', 10, 60);
    
    -- Another consumer reads from partition B only
    SELECT COUNT(*) INTO count_p2 
    FROM queen.pop('test-pubsub-partition', 'partition-b', 'partition-consumer', 10, 60);
...
-- Test 4: Consumer group ack updates cursor independently
DO $$
DECLARE
    msg_g1 RECORD;
    msg_g2 RECORD;
...
BEGIN
    -- Setup
    PERFORM queen.push('test-pubsub-ack', '{"ack": "test"}'::jsonb);
    
    -- Both groups pop
    SELECT * INTO msg_g1 FROM queen.pop('test-pubsub-ack', 'ack-group-1', 1, 60);
    SELECT * INTO msg_g2 FROM queen.pop('test-pubsub-ack', 'ack-group-2', 1, 60);
    
    -- Only group 1 acks
    ack_g1 := queen.ack_group(msg_g1.transaction_id, msg_g1.partition_id, msg_g1.lease_id, 'ack-group-1');
    
    -- Group 1 should have no more messages
    SELECT COUNT(*) INTO remaining_g1 
    FROM queen.pop('test-pubsub-ack', 'ack-group-1', 10, 60);
    
    -- Group 2 still has the message (not acked by group 2)
    -- Note: It's still leased by msg_g2, but after lease expires, it should be available
    
    IF ack_g1 AND remaining_g1 = 0 THEN
        RAISE NOTICE 'PASS: Consumer group ack advances only that groups cursor';
    ELSE
        RAISE EXCEPTION 'FAIL: Group ack issue (ack=%, remaining=%)', ack_g1, remaining_g1;
    END IF;
END;
$$;
...
BEGIN
    -- Setup: Push messages
    PERFORM queen.push('test-mode-comparison', '{"mode": "test"}'::jsonb);
    
    -- Queue mode (default __QUEUE_MODE__ consumer group)
    SELECT COUNT(*) INTO count_queue FROM queen.pop_one('test-mode-comparison');
    
    -- Pub/Sub mode (named consumer group)
    SELECT COUNT(*) INTO count_pubsub 
    FROM queen.pop('test-mode-comparison', 'named-group', 1, 60);
    
    IF count_queue = 1 THEN
        RAISE NOTICE 'PASS: Queue mode pops message (queue=%, pubsub=%)', count_queue, count_pubsub;
    ELSE
...
DECLARE
    ...
    push_ops JSONB := '[]'::jsonb;
BEGIN
    -- Setup: Push 10 messages using transaction API
    FOR i IN 1..10 LOOP
    ...
    SELECT COUNT(*) INTO count_a FROM queen.pop('test-three-groups', 'group-alpha', 20, 60);
    SELECT COUNT(*) INTO count_b FROM queen.pop('test-three-groups', 'group-beta', 20, 60);
    SELECT COUNT(*) INTO count_c FROM queen.pop('test-three-groups', 'group-gamma', 20, 60);
